    ...
    <div className="p-4 md:p-6">
          ...
          {formFields.map((key) => (
            <div key={key} className={key === "skills" ? "md:col-span-2 lg:col-span-3" : ""}>
              <label className="block text-sm font-medium mb-1 md:mb-2 capitalize">
                ...
                {["rollNo", "firstName","lastName", "email", "phone"].includes(key) && (
                  <span className="text-red-500 ml-1">*</span>
                )}
              </label>
              <input
                type={key === "profileImage" || key === "resume" || key === "certificate" ? "file" : key === "password" ? "password" : "text"}
                name={key}
                value={key === "profileImage" || key === "resume" || key === "certificate" ? undefined : formData[key]}
      ...
      <div className="bg-white p-4 md:p-6 rounded-lg shadow">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3">
          <h3 className="text-lg md:text-xl font-semibold">All Students ({students.length})</h3>
        ...
        {students.length === 0 ? (
          <div className="text-center py-8 text-gray-500">No students added yet. Add your first student above.</div>
        ) : (
          <>
              ...
              <table className="w-full border table-auto min-w-max">
                <thead>
                  <tr className="bg-gray-100">
                    ...
                    <th className="border px-2 md:px-4 py-2 text-left text-xs md:text-sm">Certificate</th>
                ...
                <tbody>
                  {currentStudents.map((s) => (
                    <tr key={s.id} className="hover:bg-gray-50">
                      ...
                      <td className="border px-2 md:px-4 py-2 text-xs md:text-sm font-medium">{s.firstName} {s.lastName}</td>
                      ...
                      <td className="border px-2 md:px-4 py-2 text-xs md:text-sm hidden xl:table-cell">{s.skills || "-"}</td>
                      <td className="border px-2 md:px-4 py-2 text-xs md:text-sm">
                        {s.profileImage ? (
                          <img src={s.profileImage} alt="Profile" className="w-8 h-8 md:w-10 md:h-10 object-cover rounded-full mx-auto" />
                        ) : (<span className="text-gray-400 text-xs">No image</span>)}
                      </td>
                      <td className="border px-2 md:px-4 py-2 text-xs md:text-sm">
                        {s.resume ? (
                          <a href={s.resume} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline text-xs md:text-sm">View</a>
                        ) : (<span className="text-gray-400 text-xs">No resume</span>)}
                      </td>
                      <td className="border px-2 md:px-4 py-2 text-xs md:text-sm">
                        {s.certificate ? (
                          <a href={s.certificate} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline text-xs md:text-sm">View</a>
                  ...
                  {getPageNumbers().map((page) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded border text-sm ${
                        currentPage === page ? "bg-blue-600 text-white border-blue-600" : "hover:bg-gray-50 transition"
                      }`}
                    >
                      {page}
                    </button>
